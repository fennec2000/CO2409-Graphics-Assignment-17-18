...
VS_NORMALMAP_OUTPUT NormalMapTransform( VS_NORMALMAP_INPUT vIn )
{
	...
	if (Wiggle)
	{
		worldPos.x += sin(modelPos.y + Wiggle) * WigglePower;
		worldPos.y += sin(modelPos.x + Wiggle) * WigglePower;
		worldPos += worldNormal * (sin(Wiggle) + 1.0f) * WigglePower;
	}
	...
	vOut.ModelTangent = vIn.Tangent + Mover;

	// Pass texture coordinates (UVs) on to the pixel shader, the vertex shader doesn't need them
	vOut.UV = vIn.UV + Mover;
